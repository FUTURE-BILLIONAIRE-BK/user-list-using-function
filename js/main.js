...
const wrappper = document.createDocumentFragment(' ')
...
const userImg = createTag('img', 'user__img card-img' )
userImg.src = data[i].src
const userName = createTag('h3', 'user__fullname h5 mt-3')
user_info(userName, `${data[i].first_name} ${data[i].last_name}`)
const userEmail = createTag('p', 'user__email h6 text-primary d-block')
user_info(userEmail, data[i].email)
const userGender = createTag('p', 'user__gender h6')
user_info(userGender, data[i].gender)
const userAddress = createTag('p', 'user__address text-bg-info rounded text-white ps-2')
user_info(userAddress, data[i].ip_address)
div.append(userImg, userName, userEmail, userGender, userAddress)
li.append(div)
wrappper.append(li)
}
ul.append(wrappper)
